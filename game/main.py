    ...
    while True:
        ...
        if action == 'new_game':
            ...
            if initial_adventurer:
                ...
                game_loop(adventurers, shared_inventory, game_state, parties, active_party)
        elif action == 'load_game':
            # Load game logic
            result = load_game()
            if result:
                adventurers, shared_inventory, game_state, parties, active_party_loaded = result
                active_party[0] = active_party_loaded[0] if active_party_loaded else None
                game_loop(adventurers, shared_inventory, game_state, parties, active_party)
            else:
...
def game_loop(adventurers, shared_inventory, game_state, parties, active_party):
    while True:
        ...
        if val == '1':
            ...
            if active_party[0]:
                inventory_management_interface(active_party[0].members, shared_inventory)
            else:
            ...
            if active_party[0]:
                ...
                if selected_dungeon:
                    dungeon_exploration_interface(active_party[0], selected_dungeon)
                else:
            ...
            save_game(adventurers, shared_inventory, game_state, parties, active_party)
            input('\nPress Enter to continue...')
        elif val == '5':
...
def load_dungeons():
    # Sample dungeon data
    dungeons = [
        {'name': 'Enchanted Forest', 'level_range': '1-3', 'size': 5, 'status': 'Unlocked', 'description': 'A mystical forest filled with magical creatures.', 'theme': 'Enchanted Forest'},
        {'name': 'Ancient Ruins', 'level_range': '3-5', 'size': 10, 'status': 'Unlocked', 'description': 'An underground dungeon with lurking dangers.', 'theme': 'Ancient Ruins'},
        {'name': 'Dark Swamp', 'level_range': '5-7', 'size': 15, 'status': 'Locked', 'description': 'Unlock by clearing Ancient Ruins.', 'theme': 'Dark Swamp'},
        {'name': 'Crystal Caverns', 'level_range': '7-10', 'size': 20, 'status': 'Locked', 'description': 'Unlock by having an adventurer reach Level 7.', 'theme': 'Crystal Caverns'},
